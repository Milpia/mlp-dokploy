...
export const provisionIdentity = async ({
	...
	const userId = await store.transaction(async (tx) => {
		...
		await tx.upsertSsoAccount({ userId: id, sub: identity.sub, idToken });
		if (!isOwner) {
...
const drizzleTx = (tx: Tx): ProvisioningTx => ({
	async createUser({ email, firstName, lastName }) {
		...
		if (!created) throw new Error("OIDC SSO: user could not be created");
		return created.id;
	},
	...
	async upsertSsoAccount({ userId, sub, idToken }) {
		...
		const linked = await tx.query.account.findFirst({
			where: and(
				eq(account.providerId, SSO_PROVIDER_ID),
...
export const drizzleProvisioningStore: ProvisioningStore = {
	...
	async findUserByEmail(email) {
		// Emails are stored as typed at registration; the identity provider's are lowercased.
...
export const findSsoIdToken = async (
	userId: string,
): Promise<string | null> => {
	const linked = await db.query.account.findFirst({
		where: and(
