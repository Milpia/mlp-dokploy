...
import { keycloakSsoConfig } from "@dokploy/server/db/schema";
...
type Row = typeof keycloakSsoConfig.$inferSelect;
...
const decryptSecret = (stored: string | null): string | null => {
	...
	try {
		...
		console.error(
			"Keycloak SSO: the stored client secret could not be decrypted; re-enter it in the settings.",
		);
		return null;
	}
};
...
const findRow = () =>
	db.query.keycloakSsoConfig.findFirst({
...
export const drizzleConfigRepository: ConfigRepository = {
	...
	async save(patch) {
		...
		const [row] = existing
			? await db
					.update(keycloakSsoConfig)
					.set(values)
					.where(eq(keycloakSsoConfig.id, existing.id))
					.returning()
			: await db.insert(keycloakSsoConfig).values(values).returning();

		if (!row) {
			throw new Error("Keycloak SSO configuration could not be saved");
		}
