...
export const readEnvOverrides = (
	...
	const mode = read(env, "KEYCLOAK_SSO_MODE");
	if (mode) {
		if ((SSO_MODES as readonly string[]).includes(mode)) {
			values.mode = mode as SsoMode;
		} else {
			errors.push(
				`KEYCLOAK_SSO_MODE must be one of ${SSO_MODES.join(", ")}; ignoring it.`,
			);
		}
	}

	const issuerUrl = read(env, "KEYCLOAK_SSO_ISSUER_URL");
	if (issuerUrl) {
		...
		if (normalized) {
			...
			errors.push(
				"KEYCLOAK_SSO_ISSUER_URL must be an absolute http(s) URL; ignoring it.",
			);
		}
	}

	const clientId = read(env, "KEYCLOAK_SSO_CLIENT_ID");
	if (clientId) values.clientId = clientId;

	const inlineSecret = read(env, "KEYCLOAK_SSO_CLIENT_SECRET");
	const secretFile = read(env, "KEYCLOAK_SSO_CLIENT_SECRET_FILE");
	if (inlineSecret) {
		...
		try {
			...
			errors.push(
				`KEYCLOAK_SSO_CLIENT_SECRET_FILE could not be read (${secretFile}); ignoring it.`,
			);
		}
	}

	const accessGroup = read(env, "KEYCLOAK_SSO_ACCESS_GROUP");
	if (accessGroup) values.accessGroup = accessGroup;

	const adminGroup = read(env, "KEYCLOAK_SSO_ADMIN_GROUP");
	if (adminGroup) values.adminGroup = adminGroup;

	const buttonLabel = read(env, "KEYCLOAK_SSO_BUTTON_LABEL");
	if (buttonLabel) {
		if (buttonLabel.length <= BUTTON_LABEL_MAX_LENGTH) {
			values.buttonLabel = buttonLabel;
		} else {
			errors.push(
				`KEYCLOAK_SSO_BUTTON_LABEL must be at most ${BUTTON_LABEL_MAX_LENGTH} characters; ignoring it.`,
			);
		}
	}

	const allowInsecure = read(env, "KEYCLOAK_SSO_ALLOW_INSECURE_HTTP");
	if (allowInsecure) {
		if (allowInsecure === "true" || allowInsecure === "false") {
			values.allowInsecureHttp = allowInsecure === "true";
		} else {
			errors.push(
				"KEYCLOAK_SSO_ALLOW_INSECURE_HTTP must be true or false; ignoring it.",
			);
		}
	}
