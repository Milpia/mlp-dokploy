...
import { keycloakAuthEvent } from "@dokploy/server/db/schema";
...
export const drizzleAuthEventStore: AuthEventStore = {
	async insert(event) {
		await db.insert(keycloakAuthEvent).values(event);
	},
	async listRecent(limit) {
		const rows = await db
			.select()
			.from(keycloakAuthEvent)
			.orderBy(desc(keycloakAuthEvent.createdAt))
			.limit(limit);
		return rows.map((row) => ({
			id: row.id,
	...
	async deleteOlderThan(cutoff) {
		await db
			.delete(keycloakAuthEvent)
			.where(lt(keycloakAuthEvent.createdAt, cutoff));
	},
};
...
export class AuthEventRecorder {
	...
	async record(event: AuthEventInput): Promise<void> {
		try {
			...
			this.logError("Keycloak SSO: could not record auth event", error);
		}
	...
	private async pruneIfDue(): Promise<void> {
		...
		try {
			...
			this.logError("Keycloak SSO: could not prune auth events", error);
		}
	}
}
