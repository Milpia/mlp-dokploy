...
const desiredMode = (services: KeycloakSsoServices, stored: SsoMode) =>
...
> = {
	incomplete:
		"Set the issuer URL, client ID and client secret before enabling Keycloak sign-in.",
	unverified:
		"Sign in once with Keycloak using the owner account before enabling SSO-only mode.",
	issuer_change_in_sso_only:
		"Switch to button mode before changing the issuer URL.",
};

export const updateKeycloakConfig = async (
	services: KeycloakSsoServices,
