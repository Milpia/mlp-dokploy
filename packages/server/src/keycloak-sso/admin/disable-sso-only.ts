...
const ENV_WARNING =
	"Warning: KEYCLOAK_SSO_MODE=sso-only is set in the environment; remove it and restart for the change to apply.";
...
export const disableSsoOnlyCommand = async (
	...
	try {
		...
		return {
			exitCode: 1,
			lines: [
				`Could not update the Keycloak SSO configuration: ${
	...
	const lines = [
		result.changed
			? "Keycloak SSO-only mode disabled. The instance is now in button mode."
			: "Keycloak SSO-only mode was not enabled. Nothing to do.",
	];
