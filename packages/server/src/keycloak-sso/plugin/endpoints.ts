...
export const TX_COOKIE = "keycloak_sso_tx";
const TX_SALT = "dokploy.keycloak-sso.tx";
...
const callbackUri = (baseURL: string) =>
	`${baseURL.replace(/\/+$/, "")}/keycloak/callback`;
...
const txCookieOptions = (baseURL: string, maxAge: number) => {
	...
	return {
		...
		path: `${url.pathname.replace(/\/+$/, "")}/keycloak`,
...
export const createKeycloakEndpoints = (
	resolveDeps: () => KeycloakEndpointDeps,
) => ({
	keycloakSignIn: createAuthEndpoint(
		"/keycloak/sign-in",
		{
			method: "GET",
	...
	keycloakCallback: createAuthEndpoint(
		"/keycloak/callback",
		{
			method: "GET",
		...
		async (ctx) => {
			...
			if (!user) {
				throw ctx.redirect(
					errorLocation("keycloak_access_denied", result.correlationId),
				);
			}
	 ...
	 * navigates to the returned URL (Keycloak's end-session page in sso-only).
	 */
	keycloakSignOut: createAuthEndpoint(
		"/keycloak/sign-out",
