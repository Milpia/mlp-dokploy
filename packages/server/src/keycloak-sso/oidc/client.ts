...
import { KeycloakLoginError, type LoginErrorCode } from "../types";
...
export const mapOidcError = (error: unknown): LoginErrorCode => {
	if (error instanceof KeycloakLoginError) return error.code;
	if (isNetworkFailure(error)) return "keycloak_unavailable";

	for (const item of errorChain(error)) {
		const code = (item as { code?: string }).code;
		const oauthError = (item as { error?: string }).error;
		if (code === "OAUTH_JWT_TIMESTAMP_CHECK_FAILED") {
			return "keycloak_clock_skew";
		}
	...
	return "keycloak_invalid_response";
};
...
const assertTransportAllowed = (settings: OidcSettings): URL => {
	...
	try {
		...
		throw new KeycloakLoginError(
			...
			"The Keycloak issuer URL is not valid",
			{ cause },
		);
	}
	if (url.protocol === "http:" && !settings.allowInsecureHttp) {
		throw new KeycloakLoginError(
			"keycloak_unavailable",
			"Plain HTTP to Keycloak is not allowed unless explicitly enabled",
		);
	}
	return url;
};
...
export const createOpenIdClient = (lib: OpenIdLib = openid): OidcClient => {
	...
	return {
		...
		async exchangeCode(settings, input) {
			...
			// Keycloak only puts groups in the ID token when the mapper's
			// "Add to ID token" switch is on; userinfo is the fallback.
			...
			try {
				...
				if (network === "timeout") {
					return {
						ok: false,
						code: "timeout",
						message: "Keycloak did not answer within 5 seconds.",
					};
				}
				if (network === "unreachable") {
					return {
						...
						code: "unreachable",
						message:
							"Keycloak could not be reached. Check the URL, DNS and TLS certificate.",
					};
				}
				...
				if (/issuer/i.test(text)) {
					return {
						...
						message:
							"The discovery document announces a different issuer. Use the exact realm URL, e.g. https://keycloak.example.com/realms/<realm>.",
					};
				}
				return {
					ok: false,
			...
			try {
				...
				if (oauthError === "invalid_client") {
					return {
						ok: false,
						code: "invalid_client",
						message: "Keycloak rejected the client ID or secret.",
					};
				}
				const network = isNetworkFailure(error);
				if (network) {
					return {
						ok: false,
						code: network,
						message: "Keycloak stopped answering while checking the client.",
					};
				}
