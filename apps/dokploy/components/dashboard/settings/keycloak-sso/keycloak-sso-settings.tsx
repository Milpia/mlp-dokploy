...
import { KeycloakAuthEvents } from "./keycloak-auth-events";

type ConfigView = RouterOutputs["keycloakSso"]["get"];
...
export const KeycloakSsoSettings = () => {
	const utils = api.useUtils();
	const { data: view, isPending } = api.keycloakSso.get.useQuery();
	const { mutateAsync: update, isPending: isSaving } =
		api.keycloakSso.update.useMutation();
	const { mutateAsync: testConnection, isPending: isTesting } =
		api.keycloakSso.testConnection.useMutation();
	...
	useEffect(() => {
		setCallbackUrl(`${window.location.origin}/api/auth/keycloak/callback`);
	}, []);
	...
	const onSubmit = async (values: FormValues) => {
		...
		try {
			...
			await utils.keycloakSso.invalidate();
