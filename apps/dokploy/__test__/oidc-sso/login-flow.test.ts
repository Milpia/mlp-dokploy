import {
	...
	startLogin,
} from "@dokploy/server/oidc-sso/plugin/login-flow";
...
const callback = (query = "code=abc&state=st") =>
	new URL(`https://dokploy.example.com/api/auth/oidc/callback?${query}`);
const redirectUri = "https://dokploy.example.com/api/auth/oidc/callback";
...
describe("startLogin", () => {
	...
	it("FR-016: maps an unreachable provider to sso_unavailable and records it", async () => {
		...
		expect(result).toMatchObject({ ok: false, code: "sso_unavailable" });
...
describe("completeLogin", () => {
	...
	it("US1-5: a cancelled login is reported as cancelled", async () => {
		...
		expect(result).toMatchObject({ ok: false, code: "sso_cancelled" });
	...
	it("rejects other provider errors as invalid responses", async () => {
		...
		).resolves.toMatchObject({ code: "sso_invalid_response" });
	});
	...
	it("NFR-SEC-002: a failed token validation is an invalid response", async () => {
		...
		expect(result).toMatchObject({ code: "sso_invalid_response" });
	...
	it("FR-006: an unverified email is reported as such", async () => {
		...
		await expect(
			completeLogin(deps, { tx, callbackUrl: callback(), redirectUri }),
		).resolves.toMatchObject({ code: "sso_email_unverified" });
		expect(recorded.at(-1)).toMatchObject({
			outcome: "denied",
	...
	it("FR-007: users outside the access group are denied and logged", async () => {
		...
		await expect(
			completeLogin(deps, { tx, callbackUrl: callback(), redirectUri }),
		).resolves.toMatchObject({ code: "sso_access_denied" });
		expect(recorded.at(-1)).toMatchObject({
			outcome: "denied",
...
describe("resolveSignOutTarget (FR-010)", () => {
	...
	it("sso-only mode ends the identity provider session too", async () => {
		const { deps, oidc } = makeDeps({
