import { TX_COOKIE } from "@dokploy/server/oidc-sso/plugin/endpoints";
...
const signIn = (auth: ReturnType<typeof setup>["auth"], query = "") =>
	auth.handler(new Request(`${BASE}/api/auth/oidc/sign-in${query}`));

describe("GET /oidc/sign-in", () => {
	it("NFR-SEC-001: redirects to the provider with a sealed, short-lived tx cookie", async () => {
		const { auth } = setup();
		...
		expect(cookie).toMatch(/Path=\/api\/auth\/oidc/i);
	...
	it("FR-016: redirects to the login page with an error when the provider is down", async () => {
		const { auth, oidc } = setup();
		...
		expect(response.headers.get("location")).toMatch(
			/^\/\?error=sso_unavailable&ref=[0-9A-F]{12}$/,
		);
	});
});
...
describe("GET /oidc/callback", () => {
	const callback = async (
		...
		return ctx.auth.handler(
			new Request(`${BASE}/api/auth/oidc/callback?${query}`, {
	...
	it("NFR-SEC-001: without the tx cookie the login fails closed", async () => {
		...
		const response = await ctx.auth.handler(
			new Request(`${BASE}/api/auth/oidc/callback?code=abc&state=st`),
		);
		expect(response.headers.get("location")).toMatch(
			/^\/\?error=sso_invalid_response/,
		);
	...
	it("FR-002: returns 404 when the feature is disabled", async () => {
		...
		const response = await ctx.auth.handler(
			new Request(`${BASE}/api/auth/oidc/callback?code=abc`),
		);
		expect(response.status).toBe(404);
	});
});

describe("POST /oidc/sign-out (FR-010)", () => {
	...
	const signOut = (ctx: ReturnType<typeof setup>, cookie: string) =>
		ctx.auth.handler(
			new Request(`${BASE}/api/auth/oidc/sign-out`, {
				method: "POST",
	...
	it("sso-only mode returns the provider end-session URL with the id token hint", async () => {
		const ctx = setup({
