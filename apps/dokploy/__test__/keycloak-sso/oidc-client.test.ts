import {
	...
	mapOidcError,
	type OidcSettings,
} from "@dokploy/server/keycloak-sso/oidc/client";
import { KeycloakLoginError } from "@dokploy/server/keycloak-sso/types";
...
describe("mapOidcError", () => {
	it("keeps codes from KeycloakLoginError", () => {
		expect(
			mapOidcError(new KeycloakLoginError("keycloak_clock_skew", "x")),
		).toBe("keycloak_clock_skew");
	});

	it("NFR-PERF-004: maps timeouts and network failures to unavailable", () => {
		expect(mapOidcError(timeoutError())).toBe("keycloak_unavailable");
		expect(mapOidcError(new TypeError("fetch failed"))).toBe(
	...
	it("maps a user cancellation to cancelled", () => {
		...
		expect(mapOidcError(error)).toBe("keycloak_cancelled");
	});
	...
	it("maps other authorization errors to invalid_response", () => {
		...
		expect(mapOidcError(error)).toBe("keycloak_invalid_response");
	});
	...
	it("NFR-SEC-002: state, nonce and signature failures are invalid responses", () => {
		...
		expect(mapOidcError("weird")).toBe("keycloak_invalid_response");
	});
...
describe("createOpenIdClient", () => {
	it("NFR-SEC-001: builds a PKCE S256 request with state and nonce", async () => {
		...
		const request = await client.createAuthorizationRequest(
			settings,
			"https://dokploy.example.com/api/auth/keycloak/callback",
		);
	...
	it("NFR-SEC-004: refuses plain HTTP unless explicitly allowed", async () => {
		...
		).rejects.toBeInstanceOf(KeycloakLoginError);
	...
	it("fails closed when no ID token is returned", async () => {
		...
		).rejects.toMatchObject({ code: "keycloak_invalid_response" });
	});
	...
	it("returns null when Keycloak has no end-session endpoint or is down", async () => {
