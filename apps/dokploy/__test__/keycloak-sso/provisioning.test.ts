import type { KeycloakIdentity } from "@dokploy/server/keycloak-sso/domain/claims";
import {
...
const identity = (
	overrides: Partial<KeycloakIdentity> = {},
): KeycloakIdentity => ({
	sub: "sub-1",
...
describe("provisionIdentity", () => {
	...
	it("FR-007: creates user, account and membership in one transaction", async () => {
		...
		expect(tx.upsertKeycloakAccount).toHaveBeenCalledWith({
			userId: "new-user-id",
			sub: "sub-1",
			idToken: "id-token",
		});
	...
	it("refuses an email already linked to another Keycloak identity", async () => {
		const { store, tx } = makeStore({
			findUserByEmail: vi.fn(async () => ({
				id: "u2",
				banned: false,
		...
		expect(tx.upsertKeycloakAccount).not.toHaveBeenCalled();
	});
	...
	it("FR-008b: never touches the owner's membership", async () => {
		...
		expect(tx.upsertKeycloakAccount).toHaveBeenCalled();
	});
