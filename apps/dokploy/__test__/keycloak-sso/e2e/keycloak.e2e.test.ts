 ...
 *     -v "$PWD/__test__/keycloak-sso/e2e/realm-dokploy-test.json:/opt/keycloak/data/import/realm.json" \
 *     quay.io/keycloak/keycloak:26.0 start-dev --import-realm
 *   KEYCLOAK_E2E=1 pnpm exec vitest --config __test__/vitest.config.ts run keycloak-sso/e2e
 *
...
import type {
	ProvisioningStore,
	ProvisioningTx,
} from "@dokploy/server/keycloak-sso/identity/provisioning";
import { createOpenIdClient } from "@dokploy/server/keycloak-sso/oidc/client";
...
const memoryProvisioningStore = (tables: { user: Row[] }) => {
	...
	const tx: ProvisioningTx = {
		...
		async upsertKeycloakAccount({ userId, sub }) {
			subs.set(sub, userId);
		},
...
const setup = () => {
	...
	const auth = betterAuth({
		...
		plugins: [keycloakSso({ resolveDeps: () => deps })],
	});
...
const login = async (
	...
	const start = await ctx.auth.handler(
		new Request(
			`${BASE}/api/auth/keycloak/sign-in?returnTo=${encodeURIComponent(returnTo)}`,
		),
	);
	store(dokploy, start);
	...
	expect(callback.startsWith(`${BASE}/api/auth/keycloak/callback`)).toBe(true);
...
describe.skipIf(!enabled)("Keycloak end-to-end (NFR-QA-002)", () => {
	...
	it("FR-006: an unverified email is denied", async () => {
		...
		expect(response.headers.get("location")).toMatch(
			/error=keycloak_email_unverified/,
		);
	});
