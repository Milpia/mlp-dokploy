import type { EnvOverrides } from "@dokploy/server/keycloak-sso/config/env";
import {
	buildEffectiveConfig,
	KeycloakConfigProvider,
} from "@dokploy/server/keycloak-sso/config/provider";
import {
	type ConfigPatch,
...
describe("KeycloakConfigProvider", () => {
	const create = (repository = fakeRepository(), now = () => 0) =>
		new KeycloakConfigProvider({
			repository,
			env: noEnv,
			isCloud: false,
	...
	it("FR-021: disableSsoOnlyMode reports an env-forced mode", async () => {
		const provider = new KeycloakConfigProvider({
