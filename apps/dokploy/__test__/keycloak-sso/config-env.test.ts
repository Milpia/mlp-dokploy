import { readEnvOverrides } from "@dokploy/server/keycloak-sso/config/env";
...
describe("readEnvOverrides", () => {
	...
	it("FR-019: reads every KEYCLOAK_SSO_* variable", () => {
		const result = readEnvOverrides(
			{
				KEYCLOAK_SSO_MODE: "sso-only",
				KEYCLOAK_SSO_ISSUER_URL: "https://kc.example.com/realms/milpia",
				KEYCLOAK_SSO_CLIENT_ID: "dokploy",
				KEYCLOAK_SSO_CLIENT_SECRET: "s3cret",
				...
				KEYCLOAK_SSO_ALLOW_INSECURE_HTTP: "false",
			},
			noFile,
		);
		expect(result.errors).toEqual([]);
	...
	it("FR-019: treats empty and whitespace-only values as unset", () => {
		const result = readEnvOverrides(
			{ KEYCLOAK_SSO_CLIENT_ID: "", KEYCLOAK_SSO_ACCESS_GROUP: "   " },
			noFile,
		);
		expect(result.values).toEqual({});
	});

	it("FR-019: reads the secret from KEYCLOAK_SSO_CLIENT_SECRET_FILE and trims it", () => {
		const result = readEnvOverrides(
			{ KEYCLOAK_SSO_CLIENT_SECRET_FILE: "/run/secrets/kc" },
			(path) => {
	...
	it("FR-019: the inline secret wins over the _FILE variant", () => {
		const result = readEnvOverrides(
			{
				KEYCLOAK_SSO_CLIENT_SECRET: "inline",
				KEYCLOAK_SSO_CLIENT_SECRET_FILE: "/run/secrets/kc",
			},
			noFile,
		);
	...
	it("FR-019: an unreadable secret file is reported and ignored", () => {
		const result = readEnvOverrides(
			{ KEYCLOAK_SSO_CLIENT_SECRET_FILE: "/missing" },
			() => {
	...
	it("rejects an unknown mode and keeps the rest", () => {
		const result = readEnvOverrides(
			{ KEYCLOAK_SSO_MODE: "always", KEYCLOAK_SSO_CLIENT_ID: "dokploy" },
			noFile,
		);
		expect(result.values).toEqual({ clientId: "dokploy" });
		expect(result.errors[0]).toContain("KEYCLOAK_SSO_MODE");
	});
	...
	it("rejects an issuer URL that is not http(s)", () => {
		for (const value of [
			...
			const result = readEnvOverrides(
				{ KEYCLOAK_SSO_ISSUER_URL: value },
				noFile,
			);
			expect(result.values.issuerUrl).toBeUndefined();
			expect(result.errors[0]).toContain("KEYCLOAK_SSO_ISSUER_URL");
		}
	});
	...
	it("rejects a non-boolean KEYCLOAK_SSO_ALLOW_INSECURE_HTTP", () => {
		const result = readEnvOverrides(
			{ KEYCLOAK_SSO_ALLOW_INSECURE_HTTP: "yes" },
			noFile,
		);
	...
	it("FR-003: rejects a button label longer than 64 characters", () => {
		const result = readEnvOverrides(
			{ KEYCLOAK_SSO_BUTTON_LABEL: "x".repeat(65) },
			noFile,
		);
