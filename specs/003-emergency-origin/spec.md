...
Desde el origen de emergencia, cualquier otra acción que dependa del control de orígenes se rechaza como hoy: el login de otro usuario, el registro, el restablecimiento de contraseña, las acciones de sesión y las de organización. Fuera del modo SSO-only, el origen de emergencia no se acepta en ninguna ruta.
...
- Tras entrar por el túnel, el owner usa el panel (proyectos, ajustes del SSO): funciona, porque esas acciones no pasan por el control de orígenes. Las que sí pasan por él (cerrar sesión desde el menú, por ejemplo) siguen rechazándose desde el origen de emergencia; el owner puede cerrar el túnel o la ventana.
...
- **FR-004**: El sistema MUST aceptar el origen de emergencia solo cuando se cumplen a la vez tres condiciones: el modo SSO-only está activo, la petición es el login con email y contraseña, y el email es el del owner. En cualquier otra petición MUST NOT aceptarlo.
- **FR-005**: El sistema MUST aceptar también el origen de emergencia en la verificación del segundo factor (código TOTP o código de respaldo) que sigue a un login de emergencia del owner, y solo en ella.
...
- **SC-002**: El 100 % de las peticiones desde el origen de emergencia que no sean el login del owner (ni su segundo factor) se rechazan en modo SSO-only.
