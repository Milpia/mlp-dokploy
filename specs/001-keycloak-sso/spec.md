...
- **FR-022**: El sistema MUST funcionar con cualquier proveedor OpenID Connect que publique un documento de discovery y emita ID tokens firmados, y MUST verificarse al menos con Keycloak, Okta, Authentik, Zitadel y Authelia. La lista de proveedores verificados, con versión y resultado por escenario, la mantiene la spec 004 (`specs/004-oidc-provider-compatibility/compatibility.md`), que añade Auth0 y FusionAuth.
...
- **FR-024**: La configuración MUST ofrecer presets para Keycloak, Okta, Authentik, Zitadel, Authelia y «Otro (OIDC genérico)», que rellenan el claim de grupos y muestran el formato de la URL del issuer de cada proveedor. Los presets solo ayudan a rellenar: no limitan la configuración. La spec 004 (FR-003) añade los presets de Auth0 y FusionAuth.
